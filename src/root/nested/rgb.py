...
if FULLSCREEN_RES == True:
    try:
        ...
        DISPLAYSURFACE = pygame.display.set_mode(std_res, 0, 32)
...
# --constants//--
"""ALL VARIABLES BELOW MAY BE MODIFIED FOR EXPERIMENTING"""
CIRCLE_GROWTH_SPEED = 4
...
"""ALL VARIABLES ABOVE MAY BE MODIFIED FOR EXPERIMENTING"""
...
# --TESTING the fullscreen function//--
for i in range(2):
    try:
...
# --Initialize Everything//--
    ...
    ring_img, ring_rect = load_image('ring_bad.png', None)
    ring_imgNew, ring_rectNew = load_image('ring_bad.png', None)
    ...
    # Paint the background color, which DOES change.
    ...
    # if paused is set to true, wait for p to be pressed again.
        ...
        while paused:
                ...
                if x_event.type == KEYDOWN and event.key == K_p:
                    paused = False
                    print "OUT OF PAUSE1"
                elif x_event.type == (KEYUP or KEYDOWN) and not event.key == K_p:
                        print x_event.dict
                        pygame.event.post(x_event)
                        paused = False
                        print "OUT OF PAUSE2"

                elif x_event.type == QUIT:
                    sys.exit()
                    pygame.quit()
    ...
    # --Spin the Ring//--
        ...
        """ # ring_rectNew.center = ring_img.get_rect().center"""
